...
const REACT_APP_CORS_URL =
  process.env.REACT_APP_CORS_URL || "https://corsanywhere.herokuapp.com";
...
function useApp() {
  ...
  const getMyFavoriteMoviesFromOMDb = async (movies) => {
    try {
      ...
      for await (const movie of movies) {
        ...
        const movieURL = `${REACT_APP_CORS_URL}/https://www.omdbapi.com?apikey=${REACT_APP_OMDB_API_KEY}&i=${movie.movie_id}`;
  ...
  const searchMovie = async (search) => {
    try {
      if (!search) {
        ...
        //Added corsanywhere for CORS bypass
        const movieURL = `${REACT_APP_CORS_URL}/https://www.omdbapi.com?apikey=${REACT_APP_OMDB_API_KEY}&s=${search}`;
        const { data } = await axios.get(movieURL);
        pageLoaderhandler(false);
  ...
  const getMovieById = async (id) => {
    try {
      ...
      //Added corsanywhere for CORS bypass
      const movieURL = `${REACT_APP_CORS_URL}/https://www.omdbapi.com?apikey=${REACT_APP_OMDB_API_KEY}&i=${id}`;
      const { data } = await axios.get(movieURL);
